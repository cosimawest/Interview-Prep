...
positionY = (x.size + y.size + 1 / 2) - positionX;

check if positionX is empty on the left or right

left? maxLeftX = INT_MIN
right? minRightX = INT_MAX
...
if(maxLeftX <= minRightY && minRightX >= maxLeftY) yay!!
else if(maxLeftX > minRightY) positionX--
else positionX++
